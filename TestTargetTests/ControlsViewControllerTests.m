...
- (void)testAccessibilityLabelLookup
{
    ...
    for ( NSString *accessibilityLabel in accessibilityLabels ) {
        FRYD.accessibilityLabel(accessibilityLabel).depthFirst().present();
    }
    }
...
- (void)testTapping
{
    ...
    for ( id accessibilityInfo in accessibilityLabels ) {
        ...
        FRYD.accessibilityLabel(accessibilityLabel).depthFirst().tap();
        FRYD.accessibilityLabel(actionConfirmation).all().count(1);
    }
}
...
- (void)testTappingDisabled
{
    ...
    for ( NSString *accessibilityLabel in accessibilityLabels ) {
        ...
        FRYD.accessibilityLabel(actionConfirmation).all().absent();
        FRYD.accessibilityLabel(@"No Action").all().present();
    }
}

- (void)testSlider
{
    FRYD_KEY.accessibilityLabel(@"Slider").depthFirst().touch([FRYTouch touchStarting:0.000
                                                                               points:40
    ...
    FRYD_KEY.accessibilityLabel(@"Slider=0").all().count(1);
    ...
    FRYD_KEY.accessibilityLabel(@"Slider=100").all().count(1);
    ...
    FRYD_KEY.accessibilityLabel(@"Slider=50").all().count(1);
}

- (void)testStepper
{
    for ( NSUInteger i = 0; i < 5; i++) {
        FRYD_KEY.accessibilityLabel(@"Increment").depthFirst().tap();
    }
    FRYD_KEY.accessibilityLabel(@"Slider=75").all().count(1);

    for ( NSUInteger i = 0; i < 5; i++) {
        FRYD_KEY.accessibilityLabel(@"Decrement").depthFirst().tap();
    }
    FRYD_KEY.accessibilityLabel(@"Slider=50").all().count(1);
}
...
{
    FRYD_KEY.accessibilityLabel(@"Switch").depthFirst().tap();
    FRYD_KEY.accessibilityLabel(@"In progress").all().count(1);

    FRYD_KEY.accessibilityLabel(@"Switch").depthFirst().tap();
    FRYD_KEY.accessibilityLabel(@"In progress").all().count(0);
...
- (void)testFoo
{
    [FRY_KEY fry_simulateTouch:[FRYTouch touchStarting:0.000
                                                         points:2
