...
- (NSArray *)viewsToIgnoreForAnimationComplete:(FRYIdleCheck *)idleCheck
{
    ...
    return FRY.lookupFirst([NSPredicate fry_matchClass:[UIActivityIndicatorView class]]).views;
}
...
- (void)testAccessibilityLabelLookup
{
    ...
    for ( NSString *accessibilityLabel in accessibilityLabels ) {
        FRY.lookupFirstByAccessibilityLabel(accessibilityLabel).present();
    }
}
...
- (void)testTapping
{
    ...
    for ( id accessibilityInfo in accessibilityLabels ) {
        ...
        FRY.lookupFirstByAccessibilityLabel(accessibilityLabel).tap();
        FRY.lookupFirstByAccessibilityLabel(actionConfirmation).count(1);
    }
}
...
- (void)testTappingDisabled
{
    ...
    for ( NSString *accessibilityLabel in accessibilityLabels ) {
        ...
        FRY.lookupFirstByAccessibilityLabel(actionConfirmation).absent();
        FRY.lookupFirstByAccessibilityLabel(@"No Action").present();
    }
}

- (void)testSlider
{
    FRY.lookupFirstByAccessibilityLabel(@"Slider").touch([FRYTouch touchStarting:0.000
                                                                           points:40
    ...
    FRY.lookupFirst([NSPredicate predicateWithFormat:@"fry_accessibilityLabel = 'Slider=0' || fry_accessibilityLabel = 'Slider=5'"]).count(1);
    ...
    FRY.lookupFirst([NSPredicate predicateWithFormat:@"fry_accessibilityLabel = 'Slider=100' || fry_accessibilityLabel = 'Slider=95'"]).count(1);
    ...
    FRY.lookupFirstByAccessibilityLabel(@"Slider=50").count(1);
}

- (void)testStepper
{
    for ( NSUInteger i = 0; i < 5; i++) {
        FRY.lookupFirstByAccessibilityLabel(@"Increment").tap();
    }
    FRY.lookupFirstByAccessibilityLabel(@"Slider=75").count(1);

    for ( NSUInteger i = 0; i < 5; i++) {
        FRY.lookupFirstByAccessibilityLabel(@"Decrement").tap();
    }
    FRY.lookupFirstByAccessibilityLabel(@"Slider=50").count(1);
}
...
{
    FRY.lookupFirstByAccessibilityLabel(@"Switch").tap();
    FRY.lookupFirstByAccessibilityLabel(@"In progress").count(1);

    FRY.lookupFirstByAccessibilityLabel(@"Switch").tap();
    FRY.lookupFirstByAccessibilityLabel(@"In progress").count(0);
...
- (void)testOnScreen
{
    UIView *s = FRY.lookupFirst(FRY_accessibilityLabel(@"Switch")).view;
    s.frame = CGRectMake(3000, 0, CGRectGetWidth(s.frame), CGRectGetHeight(s.frame));

    FRY.lookupFirst(@[FRY_accessibilityLabel(@"Switch"),
...
- (void)testVisible
{
    UIView *s = FRY.lookupFirst(FRY_accessibilityLabel(@"Switch")).view;
    s.alpha = 0.0f;
    FRY.lookupFirst(@[FRY_accessibilityLabel(@"Switch"), [NSPredicate fry_isVisible]]).absent();
    s.alpha = 0.1f;
    FRY.lookupFirst(@[FRY_accessibilityLabel(@"Switch"), [NSPredicate fry_isVisible]]).present();
    s.hidden = YES;
    FRY.lookupFirst(@[FRY_accessibilityLabel(@"Switch"), [NSPredicate fry_isVisible]]).absent();
    s.hidden = NO;
    FRY.lookupFirst(@[FRY_accessibilityLabel(@"Switch"), [NSPredicate fry_isVisible]]).present();
