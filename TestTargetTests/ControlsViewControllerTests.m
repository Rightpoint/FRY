...
- (void)testTapping
{
    ...
    for ( id accessibilityInfo in accessibilityLabels ) {
        ...
        FRY.accessibilityLabel(accessibilityLabel).depthFirst().tap();
        FRY.accessibilityLabel(actionConfirmation).all().count(1);
    }
}
...
- (void)testTappingDisabled
{
    ...
    for ( NSString *accessibilityLabel in accessibilityLabels ) {
        ...
        FRY.accessibilityLabel(actionConfirmation).all().absent();
        FRY.accessibilityLabel(@"No Action").all().present();
    }
}

- (void)testSlider
{
    FRY_KEY.accessibilityLabel(@"Slider").depthFirst().touch([FRYTouch touchStarting:0.000
                                                                               points:40
    ...
    FRY_KEY.matching([NSPredicate predicateWithFormat:@"fry_accessibilityLabel = 'Slider=0' || fry_accessibilityLabel = 'Slider=5'"]).all().count(1);
    ...
    FRY_KEY.matching([NSPredicate predicateWithFormat:@"fry_accessibilityLabel = 'Slider=100' || fry_accessibilityLabel = 'Slider=95'"]).all().count(1);
    ...
    FRY_KEY.accessibilityLabel(@"Slider=50").all().count(1);
}

- (void)testStepper
{
    for ( NSUInteger i = 0; i < 5; i++) {
        FRY_KEY.accessibilityLabel(@"Increment").depthFirst().tap();
    }
    FRY_KEY.accessibilityLabel(@"Slider=75").all().count(1);

    for ( NSUInteger i = 0; i < 5; i++) {
        FRY_KEY.accessibilityLabel(@"Decrement").depthFirst().tap();
    }
    FRY_KEY.accessibilityLabel(@"Slider=50").all().count(1);
}
...
- (void)testActivityIndicator
{
    ...
    FRY_KEY.accessibilityLabel(@"Switch").depthFirst().tap();
    FRY_KEY.accessibilityLabel(@"In progress").all().count(0);
